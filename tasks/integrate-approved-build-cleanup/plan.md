...
- [x] RF1: Nenhuma mudança local do usuário é perdida, alterada ou incluída acidentalmente em commit.
- [x] RF2: Main contém o HEAD aprovado `ae77b2a` e não contém commits posteriores da branch smoke.
- [x] RF3: Gates Rust/frontend/Deno/Playwright e checks de qualidade permanecem verdes.
- [x] RF4: Bundle instalado corresponde ao source integrado, possui `com.verbalix.desktop` e Team ID `WQ44ZM274W`.
- [x] RF5: `/Applications/Verbalix.app` passa launch smoke sem crash.
- [x] RF6: Todas as worktrees secundárias e branches locais associadas são removidas após preservação/auditoria.
- [x] CA1: `git status` final exibe somente as mudanças locais originais do usuário.
- [x] CA2: `git worktree list` final contém apenas a raiz do repositório.
- [x] CA3: `git log` confirma aprovado incluído e smoke diagnóstico excluído.
...
- [x] T2 Integrar `fix-transform-action-regression` na main sem incluir a branch smoke.
- [x] T3 Rodar gates completos e verificar arquivos/segredos/diff.
- [x] T4 Gerar e assinar bundle macOS da main integrada.
- [x] T5 Preservar instalação anterior, instalar e executar launch smoke.
- [x] T6 Remover worktrees e branches locais auditadas.
- [x] T7 Confirmar estado final e documentar a entrega.
