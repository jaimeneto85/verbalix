...
- Transformações usam `TransformLease` com CAS `Active → Claimed | Cancelled`: Candidate diferente/invalidação real cancelam o lease, e o adapter AX faz o claim somente depois da revalidação final e imediatamente antes do setter.
...
- A classificação protegida precisa ler `AXRole + AXSubrole` como uma única fronteira anterior a qualquer identifier, bounds, settable ou conteúdo; no macOS, `AXTextField` com subrole `AXSecureTextField` é o caso crítico.
...
- Eventos de foco/seleção revogam writes enfileirados por um `CausalEpoch` atômico compartilhado fora da FIFO do actor; replace/restore conferem o epoch novamente no boundary imediatamente anterior ao claim/setter, mantendo o handle AX confinado ao worker.
- Toda escrita AX recebe mutation ID do caller e grava no actor, antes do setter, snapshot, texto original/transformado, estratégia, target exato e receipt. Estados preparados/indeterminados ficam pinned; estados terminais usam TTL, replay divergente falha fechado e restore indeterminado é reconciliado por leitura sem repetir o setter.
- Eventos de foco e destruição ignoram a FIFO do actor e avançam o epoch imediatamente. Notificação de seleção só é classificada como própria por uma expectativa one-shot que coincide em mutation ID, snapshot, geração, alvo forte e conteúdo/range atuais; ausência ou divergência é externa e revoga a mutação.
- O token de correlação do AXObserver usa PID mais `AXIdentifier` completo, nunca hash. Ele só é lido depois do gate `AXRole + AXSubrole`, tem `Debug` redigido e não é serializável; alvo sem identifier não arma supressão e falha fechado como evento externo.
- Restore é monotônico: a tentativa é registrada antes do único setter e o resultado tipado termina em restored, rejected ou indeterminate reconciliado. Reconcile nunca reabre estado confirmado nem executa um segundo setter para o mesmo mutation ID.
- Idempotência terminal registra também a fase que produziu o outcome (`finish` ou `reconcile`, replace ou restore). Retry só é idempotente para a mesma fase e outcome; uma chamada cross-phase com status coincidente falha sem alterar status, TTL ou restore attempt.
- Todo reader de reconciliação, inclusive correlação do observer e restore, precisa atravessar o mesmo gate de role+subrole antes de identifier, range ou conteúdo; transição para campo seguro rejeita com zero content reads.
