...
- Readiness de overlay é vinculada a uma geração UUID emitida pelo Rust e inserida na URL. O comando valida geração, label e a `NSView` da WebView chamadora atual; ACK antigo retorna `false`. Reload gira a geração nativa e esconde a janela, falhando fechado, e retries frontend são estritamente sequenciais após `false`/erro, sem timeout por `Promise.race`.
- Rotas `?overlay=` sem a geração Rust preservam a superfície transparente, mas renderizam root vazio e nunca a aplicação principal.
