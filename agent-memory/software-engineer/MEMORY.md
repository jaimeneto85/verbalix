...
- Overlays macOS usam uma única conversão de coordenadas AX top-left para Cocoa bottom-left baseada em `NSScreen.mainScreen.frame.maxY`; seleção de tela usa `frame`, clamp usa `visibleFrame`, e `setFrameOrigin:` recebe diretamente pontos Cocoa sem `scale_factor`.
